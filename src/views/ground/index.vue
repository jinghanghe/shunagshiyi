<template>
  <div class="GoundMain">
    <div class="HeadImg">
      <div class="HeadImgText">雅思口语机经训练营</div>
      <div class="HeadImgWords">AI+真人纠音师共同帮你纠音</div>
      <div class="HeadStudy">
        <div>最新口语机经</div>
        <div>专业名师纠错</div>
      </div>
      <div class="HeadInteraction">AI精准打分</div>
    </div>
    <div class="GroundSelectl">
      为什么选择这个课程
    </div>
    <div class="GroundEnjoyQ">
      <p></p>
      <p>尊享服务</p>
      <p></p>
    </div>
    <div class="GroundProcess">
      <div class="GroundProcessO">
        <div class="GroundProcessOLeft">
          <div></div>
          <p>
            提供
            <span>“讲、</span>
            <span>学、</span>
            <span>练、</span>
            <span>评”</span>
            系统化机经学习方案
          </p>
        </div>
        <div class="GroundProcessORight">
          <div></div>
          <p>
            <span>AI+真人纠音师</span>
            共同帮助您纠正机经的回答
          </p>
        </div>
      </div>
      <div class="GroundProcessT">
        <div class="GroundProcessTLeft">
          <div></div>
          <p>
            <span>由浅入深</span>
            详解机经解题思路，深度练习，积累语料
          </p>
        </div>
        <div class="GroundProcessTRight">
          <div></div>
          <p>
            精选高频
            <span>“人、</span>
            <span>事、</span>
            <span>物、</span>
            <span>地”</span>
            机经话题，外教语音示范
          </p>
        </div>
      </div>
    </div>
    <div class="GroundSelectsP">
      <div>课程可以帮您解决什么问题？</div>
      <div>
        <div>1</div>
        <div>专业纠音老师帮您纠正机经答题语音语调</div>
      </div>
      <div>
        <div>2</div>
        <div>帮您解答机经的真正解题思路与套路</div>
      </div>
      <div>
        <div>3</div>
        <div>试用当季高频机经，强化答题技巧</div>
      </div>
    </div>

    <div class="GroundService">
      <div>课程服务人群</div>
      <div class="GroundServiceEs">
        <div></div>
        <div></div>
        <div></div>
      </div>
    </div>
    <div class="Coursecontent">
      课程内容
    </div>
    <div class="CoursecontentText">
      当季机经话题/详细视频讲解/真人题目解析
    </div>
    <div class="Img1"></div>
    <div class="Img2"></div>
    <div class="Img3"></div>
    <div class="Classprocess">上课流程</div>
    <div class="ClassImg"></div>
    <div class="ashs"></div>
    <div class="GroundBottom">
      <div class="GroundBottomTime">
        <span>拼单仅剩</span>
        <span>{{ this.ss }}</span>
      </div>
      <div class="GroundBottomSpell" @click="handlerClickPersonal">
        邀请好友拼单
      </div>
    </div>
  </div>
</template>
<script>
import "./index.css";
import { ServerIP, H5IP, isWei, GetQueryString } from "../../common/common";
export default {
  data() {
    return {
      collapsed: false,
      assemblyId: "", //拼单ID,
      curriculumId: "2", //课程Id
      ss: "",
      endTime: "",
      assemblySize: "", //拼团规模
      assemblyLength: "",
      first: ""
    };
  },
  created() {
    if (GetQueryString("token") != null) {
      //判断有没有token
      this.token = GetQueryString("token");
      localStorage.setItem("token", GetQueryString("token"));
    } else {
      this.token = "";
    }
    if (GetQueryString("curriculumId") == "null") {
      this.curriculumId = 2;
      localStorage.setItem("curriculumId", this.curriculumId);
    } else if (GetQueryString("curriculumId") != null) {
      this.curriculumId = GetQueryString("curriculumId");
      localStorage.setItem("curriculumId", this.curriculumId);
    } else if (GetQueryString("curriculumId") == null) {
      this.curriculumId = 2;
      localStorage.setItem("curriculumId", this.curriculumId);
    } else if (GetQueryString("curriculumId") == "null") {
      this.curriculumId = 2;
      localStorage.setItem("curriculumId", this.curriculumId);
    }
    if (GetQueryString("assemblyId") != null) {
      this.assemblyId = GetQueryString("assemblyId");
      localStorage.setItem("assemblyId", GetQueryString("assemblyId"));
    } else {
      this.assemblyId = "";
    }
    if (GetQueryString("platform") != null) {
      this.platform = GetQueryString("platform");
      localStorage.setItem("platform", GetQueryString("platform"));
    } else {
      this.platform = "";
    }
    if (GetQueryString("spell") != null) {
      //判断有没有token
      this.token = GetQueryString("spell");
      localStorage.setItem("spell", GetQueryString("spell"));
    } else {
      this.spell = "";
    }
    if (GetQueryString("first") != null) {
      //判断有没有token
      this.first = 1;
    } else {
    }
    this.QuerySheet(); //查询有没有支付和支付状态
    fetch(ServerIP + "v2/curriculums/assemblyInfo/" + this.assemblyId, {
      method: "GET",
      mode: "cors"
    }).then(res => {
      this.startDate = res.data.curriculumInfo.startDate;
      this.startTime = res.data.startTime;
      this.assemblePrice = res.data.curriculumInfo.assemblePrice;
      this.price = res.data.curriculumInfo.price;
      this.assemblySize = res.data.curriculumInfo.assemblySize;
      this.assemblyLength = res.data.membersInfo.length;
      this.endTime = new Date(res.data.startTime.replace(/-/g, "/")).getTime();
      this.endTime = this.endTime + res.data.assemblyInvalidTimelong;
      this.interval = setInterval(() => {
        let newData = new Date(); //开始时间
        let T = this.endTime - newData.getTime();
        if (T < 0) {
          this.ss = "拼单结束";
          clearInterval(this.interval);
        } else {
          let d = Math.floor(T / 1000 / 60 / 60 / 24);
          let h = Math.floor((T / 1000 / 60 / 60) % 24);
          let m = Math.floor((T / 1000 / 60) % 60);
          let s = Math.floor((T / 1000) % 60);
          if (h < 10) {
            h = "0" + h;
          }
          if (m < 10) {
            m = "0" + m;
          }
          if (s < 10) {
            s = "0" + s;
          }
          this.ss = d + "天:" + h + "时:" + m + "分:" + s + "秒";
          this.ss = h + ":" + m + ":" + s + "";
        }
      }, 1000);
      if (this.first == 1) {
        this.handlerClickPersonal();
      }
    });
  },
  methods: {
    handlerClickPersonal() {
      if (this.ss == "拼单结束") return;
      if (localStorage.getItem("platform") != null) {
        var u = window.navigator.userAgent.toLowerCase();
        if (u.indexOf("android") > -1 || u.indexOf("linux") > -1) {
          android.setDialog(
            this.endTime,
            this.assemblySize - this.assemblyLength,
            GetQueryString("assemblyId")
          );
        } else if (u.indexOf("iphone") > -1) {
          window.webkit.messageHandlers.MessageHandlerName.postMessage({
            timestamp: this.startTime,
            Number: this.assemblySize - this.assemblyLength,
            assemblyId: this.assemblyId
          });
        }
      } else {
        window.location.href = H5IP + "share?assemblyId=" + this.assemblyId;
      }
    },
    QuerySheet() {
      fetch(ServerIP + "v2/curriculums/purchaseInfo/" + this.curriculumId, {
        method: "get",
        mode: "cors"
      })
        .then(res => {
          return res.json();
        })
        .then(res => {
          //0已购买 1拼团中 2拼团完成 -1未支付

          if (res.data.buyState == 0) {
            window.location.href =
              H5IP + "spellsuccess?spell=2&assemblyId=" + res.data.assemblyId;
          } else if (res.data.buyState == 1) {
          } else if (res.data.buyState == 2) {
            window.location.href =
              H5IP + "spellsuccess?spell=2&assemblyId=" + res.data.assemblyId;
          }
        });
    }
  },
  mounted() {}
};
</script>
